 ...
 *
 * ✅ **Regenerated by P96/FED-002 (2026-09-18), and the red run was counted before the golden was
 * touched.** The cause is a one-line sentence, not an emitter change.
 *
 * Richard ruled that `Parse Feed` and `Parse XML` are export TARGETS rather than exemptions
 * (*"I'll likely convert all nodes to code export"*), so both were classified `scheduled` and
 * `pickerCoverageTotal` moved **128 → 130** — taken from `export-ledger:picker --check`'s own
 * printed count, not incremented by hand. Every emitted README carries `alphaNotice()`, whose one
 * sentence reads those numbers: *"118 of the 128 nodes you can place export today (92%)"* became
 * *"118 of the 130 … (90%)"*. The floor did not move, because neither parser exports yet.
 *
 *  - The red run named **49** differing files — `README.md` in **every one of the 49 projects**
 *    and nothing else. A change that lands on every project uniformly is what a global constant
 *    looks like; a node-specific change would have hit only the projects that place that node.
 *  - The sibling gate `exported-readme.test.ts` stayed GREEN throughout, because it compares the
 *    emitted numbers to the ledger's own fields rather than to a copy of them — so the pair says
 *    the README is still telling the truth, and only the truth moved.
