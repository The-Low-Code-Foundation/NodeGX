...
describe('§A the fixture, whole — two stacks, a tab bar in Replace mode, a wizard with a push and two pops', () => {
  ...
  test('A11 the ledger rows moved, so the three picker cards carry no badge; the floor is 114 after the merge (108 on its own branch)', () => {
    ...
    expect(ledger.pickerCoverageFloor).toBe(118); // GAM-013 Repeat (P88) on top of §66 Subscribe To Changes (session 90) on top of §65 WebSocket (session 89) on top of §64 Server-Sent Events (session 88) on top of §61 the component-stack trio beside §60 + §62 + §63 (session 86)
