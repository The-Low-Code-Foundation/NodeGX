...
describe('§A the fixture, whole — Query Records → Filter Records → For Each, Count → Text', () => {
  ...
  test('A6 the ledger row moved, so the picker card carries no badge', () => {
    ...
    expect(ledger.pickerCoverageFloor).toBe(118); // GAM-013 Repeat (P88) on top of §66 Subscribe To Changes (session 90) on top of §65 WebSocket (session 89) on top of §64 Server-Sent Events (session 88) on top of §61 the component-stack trio + §60 the component-object trio + §62 the relation pair + §63 Drag (session 86); §57 + §58 + §59 (session 85)
  });
});
