...
describe('§A the fixture, whole — a card in a bounded board, four live reads, three listeners, two snaps', () => {
  ...
  test('A8 the ledger row moved, so the picker card carries no badge; the floor is 111', () => {
    ...
    expect(ledger.pickerCoverageFloor).toBe(118); // GAM-013 Repeat (P88) on top of §66 Subscribe To Changes (session 90) on top of §65 WebSocket (session 89) on top of §64 Server-Sent Events (session 88) on top of §63 Drag + §61 the component-stack trio (session 86) on top of §57 + §58 + §59 (session 85)
  });
