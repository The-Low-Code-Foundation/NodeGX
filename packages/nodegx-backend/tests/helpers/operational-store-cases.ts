/**
 * The `IOperationalStore` cases, once, for any implementation (BRG-005).
 *
 * BRG-002's spec (`brg002-operational-store.test.ts`) wrote these against
 * `SqliteOperationalStore` by name. BRG-005 brings a second implementation,
 * and a mutation list — or a case list — maintained twice diverges, which is
 * the same reason `conformance/mutants.ts` lives beside the suite. So the
 * bodies moved here unchanged, and each engine's spec hands in a factory.
 *
 * Two hooks are engine-specific and the runner asks for them by name rather
 * than guessing: how to open a **second, independent** store on the same
 * table (the race case — the constraint must arbitrate, not this process), and
 * how to **break** the store so the fail-closed case can watch an error that
 * is not contention be rethrown.
 */
...
export interface OperationalStoreHarness {
  /** A fresh store over a fresh table. */
  store: IOperationalStore;
  /** A second store over the SAME table, on its own connection. */
  openSecond(): Promise<IOperationalStore>;
  /** Make the next call fail with something that is NOT a constraint violation. */
  breakStore(): Promise<void>;
  /** Release everything this harness opened. */
  close(): Promise<void>;
}

export function describeOperationalStore(name: string, open: () => Promise<OperationalStoreHarness>): void {
  describe(name, () => {
    let h: OperationalStoreHarness;
    let store: IOperationalStore;
    ...
    it('two connections to one table arbitrate through the constraint, not through this process', async () => {
      const second = await h.openSecond();
    ...
    it('settle is refused when the record is not in fromState — so it cannot fire twice', async () => {
      ...
      expect(await store.settle(MINE, 'k', 'holder', next)).toBe(true);
      expect(await store.settle(MINE, 'k', 'holder', { ...next, value: 'second', now: 300 })).toBe(false);
