...
✅ **The ledger question is RULED and paid** (R4 below) — but read the ruling before FED-003, because
it changes what `Model Request` owes: Richard, 2026-09-18, *"I'll likely convert all nodes to code
export, so you can add them to the list of exportable ones to work on."* `Parse Feed` and `Parse XML`
are now the coverage ledger's first two **`scheduled`** rows — commitments, not exemptions.
`Model Request` is a CLOUD-ONLY node, so its row is `backend-only` and the gate will *insist* on
that (`export-ledger:check` refuses a cloud-only type marked anything else). One row, no ruling
needed.
...
| R4 | ✅ **RULED AND PAID, 2026-09-18.** A new node type owes `packages/noodl-mcp` too, and neither a per-package run nor `test:main` can see it: FED-001's `net.noodl.ParseFeed` and `net.noodl.ParseXML` were unclassified against the export coverage ledger, reddening `tests/fld013ExportReach.test.ts`. **Richard's ruling:** *"In the end, to be honest, I'll likely convert all nodes to code export, so you can add them to the list of exportable ones to work on."* So both are `deferred` + **`scheduled — EXP-011 Tier 2.8`** — the ledger's FIRST two scheduled rows; every other deferral in it is a *decision*, these two are *commitments*, which is the distinction EXP-011 AC4 exists to keep. `pickerCoverageTotal` 128 → 130, taken from `export-ledger:picker --check`'s own printed count; floor holds at 118/130. `fld013ExportReach` green (11/11), and its two count gates were re-derived, not bumped — `carried.length` now reads `29 + translatedButRefusing.length`. 🔴 **The ruling's wider reach is P18's, not this phase's:** it makes the ledger's twelve remaining *"deliberately out of scope"* picker rows provisional — `Parse CSV` in particular now sits out of scope while both its siblings are scheduled. Put that to Richard as a P18 question; this phase changed only the two types it added. **Remaining in `noodl-mcp`, one clean captured run: 7 suites / 8 tests, NOT attributed here** (`cmp004Parts`, `cn004`, `nodeDocBudget`, `nodeIdAllocation`, `cmp001InterfaceDoctrine`, `d54ThemePresetIdentity`, `def038SettledTemplates`). ⚠️ An earlier run of the same suite reported **9** — two pass in a clean run, so no single count from it is worth quoting. `nodeDocBudget` is `Group` at 14,315 against 14,300, and `Group` is a visual node: `ParseFeed`/`ParseXML` cannot be what moved it. | P18 (the wider ruling) · the seven: unattributed |
