...
One directory per task, one per date: `verdicts/TVW-00N/<YYYY-MM-DD>/`. Each holds the PNGs the
session looked at, a `manifest.json` (surface, state, viewport, theme, HEAD sha, viewer bundle md5,
build kind: dev | packaged), and — for TVW-001 (the baseline) and TVW-010 — a `numbers.json` with the measured
