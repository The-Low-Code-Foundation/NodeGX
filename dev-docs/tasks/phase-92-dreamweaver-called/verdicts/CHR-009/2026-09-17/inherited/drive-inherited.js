...
(async () => {
  ...
  for (const theme of ['dark', 'light']) {
    ...
    await sleep(800);
    results.read['placeholderColor_' + theme] = JSON.parse(await ev(`JSON.stringify(${readPort('borderLeftWidth')})`));
    await zoomGroup('Border Style', `border-left-inherited-${theme}.png`);
    await zoomGroup('Corner Radius', `corner-top-left-inherited-${theme}.png`);
  }
  await ev(`document.documentElement.setAttribute('data-theme', 'dark')`);
  await sleep(600);

  // Real input: type an own value, then undo.
  await press(inputFor('borderLeftWidth'));
  results.read.focused = await ev(`document.activeElement === ${inputFor('borderLeftWidth')}`);
  await client.send('Input.insertText', { text: '5' });
  ...
  await sleep(800);
  results.read.afterType = { field: JSON.parse(await ev(`JSON.stringify(${readPort('borderLeftWidth')})`)), stored: await ev(`JSON.stringify(${model}.parameters.borderLeftWidth)`) };
  console.log('after type', JSON.stringify(results.read.afterType));
  await ev(`(() => { const u = ${mod('models/undo-queue-model.ts')}; (u.UndoQueue || u.default).instance.undo(); })()`);
  await sleep(1000);
  results.read.afterUndo = { field: JSON.parse(await ev(`JSON.stringify(${readPort('borderLeftWidth')})`)), stored: await ev(`String(${model}.parameters.borderLeftWidth)`) };
  console.log('after undo', JSON.stringify(results.read.afterUndo));

  fs.writeFileSync(path.join(outDir, 'inherited-results.json'), JSON.stringify(results, null, 2));
