...
| [CHR-012](./CHR-012-THE-COMMUNITY-TAB.md) | The Community tab, which Richard looked at after CHR-005 and ruled *"still looks like shit"* (2026-09-15). Scoped from `verdicts/CHR-005/2026-09-15/launcher-community-*.png` | Richard's look | CHR-005 |
...
| [CHR-008](./CHR-008-THE-PANEL-IS-ONE-TREE.md) 🟡 | Rows render as siblings in one React tree from those descriptors; the four decorators become props on one `PropertyRow`; the panel is keyed by node id and stops remounting; group-level gating (R8). **Slice 1 built s8: R8** — a group whose rows one switch turned off draws ONE line + `Turn on` (Box Shadow 6 sentences → 1, driven both themes, press and undo, §6.4). §3.1/3.2/3.4–3.8 not built | audit §4 | CHR-007 |
