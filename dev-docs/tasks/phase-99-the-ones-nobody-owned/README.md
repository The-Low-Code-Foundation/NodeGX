...
drive left behind, read against `cline-dev` HEAD `55dd19523`. **Status: 📋 building — HLT-001 ✅, HLT-002 ✅, HLT-003 ✅, HLT-006 ✅, six to go (plus HLT-011 and HLT-012, opened by HLT-003 and HLT-006).
Prefix: `HLT`.**
...
| [HLT-012](./HLT-012-THE-NUMERIC-FIELDS-CANNOT-OFFER-A-TOKEN.md) | 🔴 **Opened by HLT-006, 2026-09-21 — measured, not speculative.** HLT-006 gave the colour picker an enumeration; the other **102** tokens still have no surface. Measured on `Puppy test 3`: **36** distinct spacing/typography/radius tokens in use across **19** parameters — `paddingBottom`, `fontSize`, `rowGap`, `borderRadius`, `lineHeight`, `letterSpacing` — every one typed by hand or written by the AI path. The fields already **tolerate** `var()` (`NumberWithUnits`, `Dimension`, `MarginPaddingType` each paid for that in an earlier phase); none **offers** it. ⚠️ This is HLT-006's retired AC4 moved to the right surface, not deleted | HLT-006 |
| [HLT-010](./HLT-010-THE-GATE-THAT-WOULD-HAVE-CAUGHT-IT.md) | 🔴 **The structural fix, and the reason this phase is not just ten bug fixes.** Nothing in CI reads the renderer log, so 264 errors survived a green suite. Build the gate: a driven session that fails on renderer errors above a named, per-class budget, with every current class at **0**. ⚠️ Written **last** — a budget set while the counts are non-zero ships red and gets switched off. Also: fix P94's stale README header. ⚠️ **And it must run `test:main` as well as `test:ci`** — HLT-003 found a gate that only `test:main` runs sitting red for a day behind a green `test:ci`, so a renderer-error budget wired into one of them inherits exactly that blind spot | HLT-001…009, HLT-011, HLT-012 |
