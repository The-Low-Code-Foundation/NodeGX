...
drive left behind, read against `cline-dev` HEAD `55dd19523`. **Status: 📋 building — HLT-001 ✅, HLT-002 ✅, HLT-003 ✅, HLT-004 ✅, HLT-005 ✅, HLT-006 ✅ (AC5 ruled WORTHY), HLT-011 ✅, HLT-012 ✅ (AC5 awaits Richard), HLT-013 ✅, HLT-008 ✅ (P93 AC7 awaits Richard), three to go (HLT-007, 009, then HLT-010 last), plus HLT-014 (opened 2026-09-21 from the Digital Bricks Training stream).
Prefix: `HLT`.**
...
| id | task | depends on |
|---|---|---|
| [HLT-014](./HLT-014-THE-POPUP-IS-NOT-A-DIALOG.md) | 📋 **Opened 2026-09-21 from the Digital Bricks Training stream (its sprint 48), at Richard's request.** Show Popup's container is a plain `Group`: nothing in the runtime or the viewer handles Escape, sets `role="dialog"`/`aria-modal`, moves focus or returns it, and `Dismissed` deliberately means *replaced*, not *closed by the user*. So the DBT template had to build its dialog as a React kit node, which is the workaround every future template will reach for. **Decided shape:** the viewer's popup layer becomes the one owner of a modal dialog by default (semantics, `inert` behind it, focus in and back, Escape → a new `Cancelled` output with a `Close on Escape` opt-out), and the exporter's inline `div` portal is held to the same contract by one conformance drive run against both. ⚠️ Not a renderer-error class, so **HLT-010's gate does not depend on it** | — |
| [HLT-015](./HLT-015-THE-LINK-IS-SPENT-BY-A-SCANNER.md) | 📋 **Opened 2026-09-21 from the Digital Bricks Training stream (its sprint 49), at Richard's request.** `GET /auth/magic-link/callback` **consumes the token on the GET** (`oauth-routes.ts` `magicLinkCallback` → `consumeRow`), so a mail scanner that pre-fetches links spends every one before the person clicks — and anything that fetches a link is signed in. **Decided shape:** the GET peeks and renders a one-button page; only a form POST consumes. No new node, no editor change. Blocks the DBT template's L171 | — |
