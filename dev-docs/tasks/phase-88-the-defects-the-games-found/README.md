...
| R26 | (s23, GAM-027) is "do not take the cursor" a **port** on the Button, or what a Button should always do when the thing losing focus is a text field (a change to every app)? And does it belong on the other controls too? | [GAM-027](GAM-027-A-BUTTON-CAN-BE-TOLD-NOT-TO-TAKE-THE-CURSOR.md) |
| R27 | (s23, GAM-028) which shape reports the pointer: (a) a `Device` node with boolean outputs, (b) a general `Media Query` node, or (c) one port on Text Input? And what does it report during a server render? | [GAM-028](GAM-028-AN-APP-KNOWS-IT-IS-ON-A-TOUCH-SCREEN.md) |
...
| [GAM-027](GAM-027-A-BUTTON-CAN-BE-TOLD-NOT-TO-TAKE-THE-CURSOR.md) ⬜ | A button can be told not to take the cursor | GAM-011 AC7 | R26. Measured s21: a plain click moves focus to the Button and the caret to the end, so a keypad of ordinary Buttons loses the box. The cancel s21 used is a page script, not something a graph can do |
