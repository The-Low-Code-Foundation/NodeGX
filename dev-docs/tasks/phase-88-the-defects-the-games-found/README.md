...
| id | task | row | depends on |
|---|---|---|---|
| [GAM-012](GAM-012-A-FIELD-FOCUSED-AS-ITS-ROW-APPEARS-HAS-THE-CURSOR.md) 🟡 | A field focused as its row appears has the cursor, every time | D68 | R13 (s2 `bb27086de`: faults 1–2. **s21 `416462813`: fault 3** — an unmount (`nodeUnmounted`, fires nothing) split from an explicit Blur (the named node only). HEAD's Blur after a Focus signal left the cursor in the field, driven. 6 reverted arms; Dropdown identical at 11 steps. **Left:** AC6, Rocket School's keyboard drive (the peer's tree)) |
| [GAM-010](GAM-010-A-BUTTON-CAN-BE-GIVEN-THE-KEYBOARD.md) 🟡 | A Button can be given the keyboard | D59 | R11 (s21 `35e9e4ae6`: `addFocusActions` on Button, Checkbox, Radio Button, Dropdown, Slider through the tracker, the real element as target, Unchanged + editor diagnostic when not mounted. Found and fixed: a key's own click blurred the control. Chromium keyboard-only: a remounting Next 5/5. 6 reverted arms; catalog ports added only. **Left:** AC2 + AC6, Rocket School's three scripts → wires, blocked on its uncommitted tree (Richard)) |
| [GAM-011](GAM-011-A-FIELD-CAN-ASK-FOR-A-KEYPAD-AND-TAKE-A-TAPPED-KEY.md) 🟡 | A field can ask for a keypad, and take a tapped key at the caret | D60 | R12 (s21 `2e17c9ef7` (a) Input Mode + Enter Key Hint, runtime + export; `8c7f57a06` (b) Insert Text + Backspace. AC3 in Chromium: `1923`, caret 2, focus kept. 🔴 **AC2 3/5**: a keypad of one Function per key mistypes, because a Function publishes only on change — Richard. **Left:** AC2, AC7) |
