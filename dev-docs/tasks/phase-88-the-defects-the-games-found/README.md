...
| R25 | (s23, GAM-026) `Modern` is the one preset with no `--ring` token: does it gain one, or does the `#101010` fallback stay? And is a per-project ring **width** owed? | [GAM-026](GAM-026-EVERY-CONTROL-A-KEYBOARD-REACHES-SHOWS-WHERE-IT-IS.md) |
...
| [GAM-026](GAM-026-EVERY-CONTROL-A-KEYBOARD-REACHES-SHOWS-WHERE-IT-IS.md) ⬜ | Every control a keyboard reaches shows where it is | P41 ACC-001 | R25. **s23 wrote it** as the remainder of the ring Richard ruled first (Button, deprecated Checkbox/Radio, Select, both Ranges done in `5e91dc469`): the current Checkbox/Radio hide their input at `opacity: 0` and draw nothing, the new Select is unmeasured, a click-only arm and the 3:1 check are owed. Richard put it in P88, not P41 |
