...
| R24 | (s23, 2026-09-17, asked in plain words) **"Checking on the rendered page is enough."** A template whose sentences are all wired (Rocket School, 59 Texts of 60) raises no `text-cannot-wrap` from the static door, and the validator will **not** follow a wire to a word table. The render door, run where the sentence exists, is the answer. GAM-020 AC6 ✅ | GAM-020 |
| R28 | (s23, 2026-09-17, asked in plain words: nine workarounds the templates carry for defects now fixed) **GAM-001 keep with a comment · GAM-002 change `'' + …` back to `String(…)` · GAM-003 put the Expression back (so the one-source gate relaxes) · GAM-006 "I dunno" → builder kept the pins · GAM-007 the gate follows the new reserved-name list · GAM-008 "Dunno" → builder drops `cdKick`/`cdOne` · GAM-014 keep the Group wraps · GAM-015 `padPx` goes for the shipped `readPx` · GAM-018 the pixel game takes confetti.** Each task's §8 s23 has its own clause and what it owes | GAM-001/002/003/006/007/008/014/015/018 |
...
| R17 | (s19) **Signals: the bridge makes them work (s1). Size: document the wrapper Group (z3)** | GAM-017 |
...
| R11 | Does a control's Focus go through the viewer's focus tracker, or straight to the element? | [GAM-010](GAM-010-A-BUTTON-CAN-BE-GIVEN-THE-KEYBOARD.md)  **s23: the focus ring** (ruled): `style.css` deleted every control's outline; a 3px `var(--ring)` ring on `:focus-visible`, `ringNext`/`ringTab` 7 red → 0, arm 6 red (`5e91dc469`). |
...
| R17 | ✅ **Ruled s19: s1 + z3**. ~~Should kit signal props be fixed in the bridge or refused in the types with a check? Should kit size come from `frame`, from default Width/Height ports, or be documented as a wrapper Group?~~ | [GAM-017](GAM-017-A-KIT-NODE-TAKES-A-SIGNAL-AND-A-SIZE-THE-WAY-A-BUILT-IN-DOES.md) |
| R18 | ✅ **Ruled s19: warn above a length threshold measured from the corpus**. ~~Is a short multi-word heading in a content-sized Text a finding: warn on any sentence, warn above a length threshold, or info only?~~ | [GAM-020](GAM-020-A-SENTENCE-THAT-WILL-NEVER-WRAP-IS-FLAGGED-BEFORE-A-PERSON-SEES-IT-CLIPPED.md)  **s23 AC6:** render door names the old helper's "Tu as atteint la planète !" 353px in 312 (390 FR, at the race end); static door silent: 59/60 Texts wired → Richard.  **R24 ruled s23: the rendered page is enough** — the static door will not follow a wire; AC6 closed. |
...
| R14 | ~~AC1 finds the door wrote no visual root: refuse, warn, or treat an unknown root type as visual?~~ **Moot (s15):** AC1 excluded (A); the door writes the root | [GAM-014](GAM-014-A-KIT-NODE-DRAWS-WHEN-IT-IS-THE-WHOLE-COMPONENT.md)  **s23: AC6 ruled (R28) — the wraps stay, the reason is now the size decision, not D53; no gate pinned them. Task closed.**  **s23: AC6 ruled (R28) — the wraps stay, the reason recorded as the size decision; no gate pinned them. Closed.** |
...
| R25 | ✅ **Ruled s24, both halves: "yes" to a Modern ring and "yes" to a width token.** 🔴 The first half needed no change — Modern's preset file is empty *because Modern IS the defaults*, and the defaults declare `--ring: #2563eb`; a deployed project declaring no ring of its own reads it. The width half is built: **`--ring-width`, `3px`**, and both rules read `var(--ring-width, 3px)`. ~~(s23) `Modern` is the one preset with no `--ring` token: does it gain one, or does the `#101010` fallback stay? And is a per-project ring **width** owed?~~ | [GAM-026](GAM-026-EVERY-CONTROL-A-KEYBOARD-REACHES-SHOWS-WHERE-IT-IS.md) |
| R26 | ✅ **Ruled s24: a `Keeps Focus` port on the Button, default off — and Button only.** "Always" was offered and declined, so a click still moves focus to a Button in every existing app; the other `-2` controls do not get the port. ~~(s23) is "do not take the cursor" a **port** on the Button, or what a Button should always do…? And does it belong on the other controls too?~~ | [GAM-027](GAM-027-A-BUTTON-CAN-BE-TOLD-NOT-TO-TAKE-THE-CURSOR.md) |
| R27 | ✅ **Ruled s24: (a) a `Device` node with named boolean outputs, and it reports `false` during a server render** (the client corrects it on arrival — an output that never arrives is GAM-013 s20's never-quiet render). A new built-in node's full surface is the accepted price. ~~(s23) which shape reports the pointer: (a)…(b)…(c)…? And what does it report during a server render?~~ | [GAM-028](GAM-028-AN-APP-KNOWS-IT-IS-ON-A-TOUCH-SCREEN.md) |
...
| id | task | row | depends on |
|---|---|---|---|
| [GAM-012](GAM-012-A-FIELD-FOCUSED-AS-ITS-ROW-APPEARS-HAS-THE-CURSOR.md) 🟢 | A field focused as its row appears has the cursor, every time | D68 | R13 (s2 `bb27086de`: faults 1–2. **s21 `416462813`: fault 3** — an unmount (`nodeUnmounted`, fires nothing) split from an explicit Blur (the named node only). HEAD's Blur after a Focus signal left the cursor in the field, driven. 6 reverted arms; Dropdown identical at 11 steps. **Left:** AC6, Rocket School's keyboard drive (the peer's tree)) **s22: AC6 met** (Rocket School keyboard drive, 10 cells ALL PASS, `focusIn`). |
| [GAM-010](GAM-010-A-BUTTON-CAN-BE-GIVEN-THE-KEYBOARD.md) 🟢 | A Button can be given the keyboard | D59 | R11 (s21 `35e9e4ae6`: `addFocusActions` on Button, Checkbox, Radio Button, Dropdown, Slider through the tracker, the real element as target, Unchanged + editor diagnostic when not mounted. Found and fixed: a key's own click blurred the control. Chromium keyboard-only: a remounting Next 5/5. 6 reverted arms; catalog ports added only. **Left:** AC2 + AC6, Rocket School's three scripts → wires, blocked on its uncommitted tree (Richard)) **s22: AC2 + AC6 met** — 4 focus scripts (incl. the Hunt's `hpFocusNext`) → `didMount → focus` wires; keyboard-only 20 rounds × 10 cells ALL PASS; focus ring not visible (ACC-001). |
| [GAM-026](GAM-026-EVERY-CONTROL-A-KEYBOARD-REACHES-SHOWS-WHERE-IT-IS.md) 🟢 | Every control a keyboard reaches shows where it is | P41 ACC-001 | **s24: built.** R25 ruled and applied. The ring is drawn on the **wrapper** (`.ndl-controls-pointer:has(> …:focus-visible)`), not a sibling: the hidden input is a direct child of the visible box, for the Dropdown too (its `<select>` carries no class of its own). Driven on a deployed page: 3 red → ALL PASS, screenshots looked at; mouse arm 10/10; 4 reverted arms red; contrast ≥ 3:1 everywhere (worst 4.72); gate 9 → **13/13**; Rocket School `--keys` ALL PASS. **Owed:** AC7's prefab/template corpus (`library/prefabs/form` needs a page first). 🔴 AC8's Rocket School checkbox **does not exist** — the template has no Checkbox or Radio at all |
| [GAM-011](GAM-011-A-FIELD-CAN-ASK-FOR-A-KEYPAD-AND-TAKE-A-TAPPED-KEY.md) ✅ | A field can ask for a keypad, and take a tapped key at the caret | D60 | R12 (s21 `2e17c9ef7` (a) Input Mode + Enter Key Hint, runtime + export; `8c7f57a06` (b) Insert Text + Backspace. AC3 in Chromium: `1923`, caret 2, focus kept. 🔴 **AC2 3/5**: a keypad of one Function per key mistypes, because a Function publishes only on change — Richard. **Left:** AC2, AC7) **s22: AC2 met** — Richard: "whatever actually fixes it": a Function publishes an unchanged write with a signal of the same run; touch keypad 5/5 at 1024 and 390. **s23: AC7 recorded** — the kit stays (AC2 kit-only; AC1 needs a coarse-pointer fact, AC5 a Button that keeps focus). |
| [GAM-013](GAM-013-SOMETHING-CAN-HAPPEN-EVERY-SECOND-WITHOUT-A-SCRIPT.md) 🟢 | Something can happen every second without a script | D40 | R1 (s20: `Repeat` built on `timerScheduler`: no drift, no catch-up burst, own running guard, delete stops it. AC1 census RED at HEAD (12 clock files, nothing fires twice; Delay 1). 15 spec rows, 5 reverted arms (2/1/1/4/1). Catalog, enrichment, stopwatch example, picker, cloud library, docs, CHR-007. AC2 in Chromium: 5 at 5 s, +3 in 3 s after 3 round trips. AC5: `client-only`, because `partial` kept every server render from going quiet (100/100 turns). AC7 translated (STREAM_NODES member, 32 tests, 19 arms, ledger 118/128, HLS-001 46 READMEs counted). **Left:** exported app browser drive; AC8 builds (Rocket School)) |
| [GAM-027](GAM-027-A-BUTTON-CAN-BE-TOLD-NOT-TO-TAKE-THE-CURSOR.md) 🟢 | A button can be told not to take the cursor | GAM-011 AC7 | **s24: built.** R26 ruled — a `Keeps Focus` port, default off, Button only. `Button.tsx` cancels `mousedown` (composed onto the `controlEvents` handler, never appended: FH-015). Driven as a **control pair on one page** at 1024×768 and 390×844: the key with the port keeps the caret (`192`, caret 2, then a typed `5` → `1952`); the key without it loses focus to the `<button>` and the typed `5` goes nowhere. Spec 8/8 (jsdom grades the cancel, not focus — it never moves focus on a press); reverted arm 1 red. **Owed:** the export reports the port as dropped, so an exported keypad still steals the caret (P18) |
| [GAM-028](GAM-028-AN-APP-KNOWS-IT-IS-ON-A-TOUCH-SCREEN.md) ⬜ | An app knows it is on a touch screen | GAM-011 AC7 | R27. The kit reads `(pointer: coarse)` and the product cannot, which is why the answer pad is a kit node. With GAM-027 it is the other half of GAM-011 AC7 |
...
| [GAM-015](GAM-015-A-WIRED-SIZE-REACHES-A-KIT-NODE-AS-A-SIZE.md) 🟢 | A kit node reads a wired size as the size it was sent | D65 | R15 (s20: (a) built. AC1 RED through the real bridge and in Chromium: a wired **and a typed** 40 both reach the component as `"40px"`, and `Number()` draws 64. `readPx` in the scaffold (one source, emitted), types + docs page, all 7 types copies were stale, 5 refreshed and gated. 3 reverted arms (6/suite/3). Kit gate re-pointed at `"40px"`. **Left:** AC3 editor canvas; game-kit adopting `readPx` + RKT-011 re-drive (Rocket School's byte-identical copy, the peer's)) |
| [GAM-017](GAM-017-A-KIT-NODE-TAKES-A-SIGNAL-AND-A-SIZE-THE-WAY-A-BUILT-IN-DOES.md) 🟢 | A kit React node takes a signal and a size the way a built-in node does | D70 | R17 (s22: s1 + z3 built. AC1 RED at HEAD in jest and on a deployed page: a signal **prop** stays `undefined` after 2 clicks and logs at registration, while the `inputs` + `valueChangedToTrue` control counts 1, 2, so **D70's "a React node cannot take a signal" is false**; only the prop form was dead. Bridge: a signal prop is a count from 0, edge-triggered, re-rendered. Types + 5 copies, docs sample compiled, wrapper Group documented. 5 reverted arms (4/2/1/3/1). Deployed: wired 0→1→2, unwired 0, 0 errors. AC5: 0 signal props and 0 `frame` in 28 library kits + 3 template kits. 🔴 Found: the React export drops a Click into a kit signal for both routes (a report note, nothing in the code). **Left:** AC4 editor half; AC7 (Rocket School's Boost, Richard)) **s22 later:** the export keeps a Click into a kit signal (`99522fd72`); AC7 Boost is a signal in Rocket School (`7669e3303`). **s24: AC4's editor half done.** In the canvas: the wired kit node counted 0→1→2 on the Button's Click while the **unwired** one on the same page stayed 0, and the Ports tab names it `Play … from Fire · Click`. Size per R17's z3 — the kit has no size ports (its whole panel is Variant/Mounted/CSS) and a Width typed on the wrapping Group lands: 320% → 3162px, exactly 3.2× the 988px parent |
| [GAM-016](GAM-016-THE-FONT-A-PRESET-NAMES-IS-THE-FONT-THE-PAGE-DRAWS.md) 🟢🔴 | The font a preset names is the font the page draws | D69 | R16 + addendum (s22: (a) + (c) built. AC1 RED on deployed pages: Playful/Enterprise/Soft loaded **no** face on the editor and MCP routes, while computed style named it; Modern + Inter loaded Inter. Built: 3 OFL font folders (75/55/89 KB), `planPresetFonts` shared by the editor's new-project path and MCP `set_style_preset`, switching removes an **untouched** old folder only (ruled s22), Enterprise → Source Sans 3, `font-face-not-shipped` in `validate_project`. 10 reverted arms. After: every preset loads its face, 0 foreign requests, Button + Text Input included. AC5: 50/53 template/prefab source folders fire, 47 on Inter the installer adds; real: members-area `Source Sans Pro`, story-engine `Iowan Old Style`. **Left:** editor canvas/wizard drive; members-area; MCP bundle) **s22 later:** story-engine ships Source Serif 4, members-area Source Sans 3, Rocket School's Nunito comes from the preset. **🔴 s24, AC3's editor half driven for the first time and RED:** a **Playful** project made through the Guided Setup wizard ships `preset-font-nunito` and loads its stylesheet, but the canvas computes **Inter** and `--ring: #2563eb` — Modern's defaults — with Nunito `unloaded` and the project file carrying **zero** design tokens. The preset's FONT is placed and its TOKENS never are: the launcher's pending id is peeked (`LocalProjectsModel.ts:321`) and `StyleTokensModel._applyAndClearPendingPreset` never lands it. A one-shot, so reopening cannot fix it. The unit gate calls peek and consume in the same test, so it is blind to this |
...
| [GAM-023](GAM-023-A-DEPLOY-REFUSES-A-BROKEN-WIRE-AND-KEEPS-EVERY-GOOD-ONE.md) 🟢 | A deploy publishes every wire and names the broken ones | D48 | R20 (c). s18 built: `noodl-preview/src/wireHealth.ts`. At HEAD the validation gate already refused built-in-declaration wires; 6 of 10 kinds shipped silently. Now: editor adapters + the runtime's own import sequence, health on a **second model** so the export is byte-identical on 7/7 templates, broken wires named in `warnings`, kit wires counted unchecked. 7 mutants, spec 6/6. Committed `624b054f2`. s19: R23 enum → string built (Rocket School's 1 broken → 0), spec 7/7, 1 mutant. **Left:** a release |
| [GAM-024](GAM-024-THE-DEPLOY-CENSUS-REPORTS-ONLY-REAL-DROPS.md) ✅ | The deploy census reports only real drops | D44 + D52 | s18 built with GAM-023, one port pass. story-engine 3→0, landing 11→0, members 33→0, todo 24→0, todo-demo 18→0; D52's 3 and D44's 46 come back under their arms by name. Committed `624b054f2`. AC5 closed by R21 (s19) |
| [GAM-020](GAM-020-A-SENTENCE-THAT-WILL-NEVER-WRAP-IS-FLAGGED-BEFORE-A-PERSON-SEES-IT-CLIPPED.md) 🟢 | A sentence that will never wrap is flagged before a person sees it clipped | D58 | R18 ruled s19. s19 built both doors: validator at a 26-char line (6 corpus firings, 0 headings), render against the parent box; pixel-game footer fixed (429 in 358 → wraps). **Left:** AC6 (Rocket School, peer) |
