...
| R5 | The only main view | **The week.** *"Seeing the weekly view made me think 'aaah this is what I need'."* Columns Mon–Sat, blocks per project, envelopes strip above, cash strip below. **It must fit above the fold on a laptop and never grow.** No tabs. No second page. **Amended 2026-09-21 by R5b (the page scrolls when the week does not fit) and R5a (1100px max width) — see [TPL-010-R2](TPL-010-R2-FIRST-USE.md).** |
...
| R7 | Moves | **One line of chips under the envelopes, sorted by urgency, dormant projects included.** Each project carries one *next move* with a worth and a when. Clicking a chip puts 30 minutes into the next day with room; the chip then reads as placed. **Amended 2026-09-21 by R7a (the strip wraps) and R7b–d (a move takes a day and hours; placed = a live block on or after today) — see [TPL-010-R2](TPL-010-R2-FIRST-USE.md).** |
...
| AC9 | The demo at `https://nodegx.io/templates/planner/` is driven with real clicks: AC3, AC5 and AC7 pass against the public URL, and reload keeps the state in `localStorage` | 🟢 **done, 2026-09-22.** Published with the **production** viewer (no `--allow-development-engine`) and driven **against the public URL**: `drive-tpl010-r25.js` 25/25 and `drive-tpl010-money.js` 40/40 on `https://nodegx.io/templates/planner/`, 0 console errors, state kept under `nodegx-planner-demo-v3`. ⚠️ The money drive failed its first clause once on the very first cold visit and passed on every run after: the seed is written on first read, and that one check looked before the strip had settled. Worth a longer wait in that drive if it recurs. The homepage lists it as the first featured demo, and it is on the community shelf as `planner` (data-app, 211 files, v2). |
