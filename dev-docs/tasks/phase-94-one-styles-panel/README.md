...
| [STY-006](./STY-006-WHERE-ITS-USED.md) | **Where it's used** — a Look names what wears it and takes you there | 🟡 **AC1–AC4 + AC6's gate half green (s9)** — pressing a row's count opens the list of what names that style, inline under it, and pressing an entry switches the canvas to that component and selects that node. 🔴 **The count is now a `.length` over the wearer walk** — `styleUsageIn` and `variantWearerCounts` both derive from it, so a row cannot say `9×` above eight lines. `tests-unit/sty-006` 41 tests. AC5/AC7 driven, AC8 is Richard's look |
