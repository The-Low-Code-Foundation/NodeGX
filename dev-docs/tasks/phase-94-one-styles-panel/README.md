...
| [STY-002](./STY-002-THE-LOOK-MODEL.md) | **The Look model** — one concept end to end; `Preset`/`Size` removed | 🟡 **AC2/3/4 model + AC6 green (s4); AC5 and AC1's mechanism closed (s5)** — the `Preset`/`Size` rows and the size axis are deleted, and nothing writes `_variant`/`_size` any more. ⬜ **AC7 is the next session's first job**, and 🟢 **AC1's name was RULED at s10: the concept stays a LOOK** (the rename cost was measured first — ~107 source files, 83 docs) |
| [STY-003](./STY-003-THE-PROPERTY-PANEL.md) | **The property panel** — the four rules and three states. The "clear AF" task | 🟢 **CLOSED s10 — AC8 RULED WORTHY.** (Built s5: AC1–AC3, AC6 and AC7 are in the tree: every row says where its value came from, an override says what the Look wanted and offers a revert, the menu is design §4's three sections, and the `⋯` is visible. AC5 was driven in s5 and the gutter defect Richard found was fixed and re-shot in s7.) **Richard was shown `shots/sty003-gutter-{dark,light}.png` at s10 and closed it.** |
...
| [STY-005](./STY-005-THE-STYLES-PANEL.md) | **The Styles panel in the rail** — colours, text, Looks (R1, R3–R6 as ruled) | 🟢 **CLOSED s10 — AC8 RULED WORTHY.** (AC1–AC7 green and DRIVEN s8 — the panel is in the rail between Components and Search, not `experimental`; four sections; every row badged with its layer and counted; one visible `⋯`; the old `design-tokens` panel retired without losing its token editor. `shots/sty005-panel-{dark,light}.png` went in front of him at s10.) |
| [STY-006](./STY-006-WHERE-ITS-USED.md) | **Where it's used** — a Look names what wears it and takes you there | 🟢 **CLOSED s10 — AC8 RULED WORTHY.** (AC1–AC4 + AC6 s9 — pressing a row's count opens the list of what names that style, inline under it, and pressing an entry switches the canvas to that component and selects that node. 🔴 **The count is now a `.length` over the wearer walk** — `styleUsageIn` and `variantWearerCounts` both derive from it, so a row cannot say `9×` above eight lines. `tests-unit/sty-006` 41 tests. AC5/AC7 driven 28/28; `shots/sty006-used-by-{dark,light}.png` went in front of him at s10.) |
| [STY-007](./STY-007-THE-AFTER-PICTURE.md) | **The after picture** — both themes, Richard's WORTHY | 🟢 **CLOSED s10 — AND IT CLOSES THE PHASE.** Four before/after pairs on a copy of `Todo list`, the project STY-001 photographed, full editor windows, both themes. Drive `drive-sty007-after-picture.js` **23/23 arms, exit 0**. 🔴 **Two defects fixed on the way, both inside the phase's own promise**: the Look menu never opened on a list on a project with no Looks of its own, so the twelve shipped Looks were unreachable (blocked this task's AC1); and every `var(--…)` swatch in the colour picker painted **nothing** — found by RICHARD, in the after-picture |
