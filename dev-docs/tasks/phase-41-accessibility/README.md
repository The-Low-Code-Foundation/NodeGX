...
|---|---|---|
| **ACC-001** — The focus ring exists | Replace the `outline: none` rules in `assets/style.css` with `:focus-visible` styling drawn from the design tokens, meeting 3:1 against adjacent colours. Includes the Select "alternate element" the comment promised. **Partly done in P88 s23 (2026-09-17, GAM-010 §8):** a 3px `var(--ring)` ring on Button, deprecated Checkbox/Radio, Select and both Ranges, driven on Rocket School. Left: the current Checkbox/Radio (`-2`, input at `opacity: 0`), the 3:1 check per preset. | WCAG 2.4.7, 2.4.11; Opquast 165 |
