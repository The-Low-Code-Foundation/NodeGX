...
| AC6 | ✅ (s2) **DRIVEN GREEN, FR 390×844 and EN 1366×768.** `--scenario ply004`, 24/24, exit 0. The setup carries the Maths/Typing row; in Typing the monster asks LETTERS (`d`, under *les touches de la rangée du milieu*) with no digit or operator in the prompt, the 41-key letter keyboard is drawn with the next key lit, and the lane shows three hearts and the creep pips. |
| AC7 | ✅ (s2) **DRIVEN GREEN, both arms**, measured on where the monster actually stands (`.rkt-monster` along `.rkt-lane`). Three wrong answers bring it in; then three SLOW right answers hold it at exactly the same place three times over (274 px at 390×844, 556 px at 1366×768 — 0 px of movement); one QUICK right answer pushes it back (274 → 364, 556 → 646). 🔴 The ●○○○ row is NOT the creep — it is hits landed on the monster, and it falls on every right answer, slow or quick. A first reading of this drive graded those pips and made slow and quick look identical. |
