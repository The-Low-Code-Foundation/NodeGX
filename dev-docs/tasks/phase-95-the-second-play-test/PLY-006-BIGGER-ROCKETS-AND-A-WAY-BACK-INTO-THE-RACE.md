...
| AC7 | Engine gate: layer 3 is off in a two-player race, and a bought turbo is consumed exactly once. |
| AC8 | Drive, FR 390×844 and EN 1366×768: lose the first four answers on purpose, then answer three right → the pips fill → the turbo button appears → fire it → the next right answer moves the rocket **twice** the distance the line says a plain one would, and the line says why. **Read on the screenshot.** |
