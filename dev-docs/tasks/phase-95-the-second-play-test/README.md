...
| R6 | **The buy card sits over the shelf, and closing it returns the child to the tab they were on.** | The hangar. Both halves had one cause: asking UNMOUNTED the shelf, and remounting re-fired the node whose job is to open on the Face tab — so every "No thanks" threw a child browsing rockets back to Face. ✅ Driven: the card is centred over the tiles, and after "Non" the shelf is still on the Rocket tab with its scroll kept. 🔴 It took two goes on the screen: the CSS must be `!important` (a Group writes `position` inline) and `fixed`, not `absolute` (pinned to a 17-tile shelf the card sat at y = -47 for a child who had scrolled). |
