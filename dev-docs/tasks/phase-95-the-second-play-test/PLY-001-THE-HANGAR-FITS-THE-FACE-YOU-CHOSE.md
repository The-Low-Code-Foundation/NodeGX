...
| AC8 | 🟡 **BUILT (s3), NOT YET DRIVEN.** s2 drove this RED and found the missing half was never written: §3.4's third bullet existed in no source and no built file, so a `thumbs` profile's face tab drew **nothing at all** — no tiles, no sentence. Built now, in both languages, naming the face as the chooser names it ("The Thumbs face doesn’t wear things. Change your face in the player menu to dress up." / "La tête Thumbs ne porte rien. Change de tête dans le menu du joueur pour t’habiller."). 🔴 **The condition is read off the SHELF, never from a copied list of dropped looks:** the sentence appears when no face item on offer fits this look, so adding a wearable row for `thumbs` one day, or changing `HANGAR_LOOKS`, needs no change here and cannot go stale. `LOOK_NAMES` moved to `tpl007Curriculum.ts` so the chooser and the shelf read ONE copy of each face's name. Gated (engine + built artefact) with a sabotage arm, and the Text that draws it is asserted NOT content-sized — it is a sentence and must wrap. Driven: a dropped look's face tab shows the sentence and no tiles; its rocket tab is untouched and silent; each kept look shows tiles and says nothing. |
