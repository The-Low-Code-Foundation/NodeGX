...
| AC8 | ✅ **GREEN, DRIVEN (s3), both languages.** s2 drove this RED and found the missing half had never been written: §3.4's third bullet existed in no source and no built file, so a `thumbs` profile's face tab drew **nothing at all** — no tiles, no sentence. Built and driven: a dropped look's face tab shows *"The Thumbs face doesn’t wear things. Change your face in the player menu to dress up."* / *"La tête Thumbs ne porte rien. Change de tête dans le menu du joueur pour t’habiller."*, draws no tiles, and its rocket tab still works and stays silent. 🔴 **The condition is read off the SHELF, never from a copied list of dropped looks:** the sentence appears when no face item on offer fits this look, so a wearable row added for `thumbs` one day, or a change to `HANGAR_LOOKS`, needs no change here and cannot go stale. `LOOK_NAMES` moved to `tpl007Curriculum.ts` so the chooser and the shelf read ONE copy of each face's name. Gated on the engine and on the built artefact, with a sabotage arm, and the Text that draws it is asserted NOT content-sized — it is a sentence and must wrap. |
