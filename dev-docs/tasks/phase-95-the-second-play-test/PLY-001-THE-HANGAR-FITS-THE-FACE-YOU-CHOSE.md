...
| AC2 | Engine gate: for each of the three kept styles, the rows `HANGAR_SHELF_SCRIPT` returns for `tab: 'face'` are **all** items that fit that look, and **no** item that does not. Sabotage: restore the `!fits` branch, and the arm goes RED. |
| AC3 | Template gate: every kept style has **≥ 10** face items, and every row's `part` + `value` exists in the installed DiceBear schema for each look it lists — read from `node_modules/@dicebear/<look>/lib/schema.js`, never a copied list. Sabotage arm. |
| AC4 | Kit gate: for every face item on the new shelf, the SVG with the option differs from the SVG without it, on every look it claims (RKT-011 AC3's method, including its coincidence arm for a seed that already draws that part). |
| AC5 | Engine gate: a profile that owns items for look X, moved to look Y and back to X, still owns every one and wears the same ones it wore. `owned` only ever grows. Sabotage: prune `owned` on a look change, and the arm goes RED. |
| AC6 | Engine gate: the chooser's styles are the three kept ones for a new player, and four for a profile whose `look` is `thumbs` or `fun-emoji` — the fourth being their own. |
| AC7 | Drive, FR 390×844 and EN 1366×768: create an adventurer player, open the hangar → **no greyed "fits another face" tile anywhere**, ≥ 10 tiles, each ≥ 44×44 px by touch, no sideways scroll. Read on the screenshot, not only in text. |
| AC8 | Drive: a `thumbs` profile restored from a save code still draws its face everywhere, its face tab says the sentence in §3.4, and its rocket tab works. |
