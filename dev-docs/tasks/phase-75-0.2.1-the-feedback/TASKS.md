...
## 🔴 Triage against 0.2.2 — what rides, what rolls (2026-08-31, phase 82 REL-005)

Richard ruled on 2026-08-31 that **the next app cut is `0.2.2` and this phase rolls forward**: its
remainder is triaged, not force-marched into the release. This section is that triage, and it is
[phase 82](../phase-82-0.2.2-the-first-row-on-the-shelf/TASKS.md)'s REL-005.

🔴 **One distinction decides every row. `0.2.2` is an APP cut** — the Electron editor, tagged from
this repo. **`nodegx-community` is SERVED**: its work is either deployed to nexus-1 or it is not, and
it never rides a tag. A row with an editor half and a platform half is therefore **triaged twice**,
and "done" on the served side is a *deploy stamp*, never a commit.

**Re-counted from tiers 0–4 on 2026-08-31: 26 rows — 22 ✅ done, 4 open.** ⚠️ Phase 82's README §1.5
and its REL-005 detail both said *"24 done, 4 open"*; the done count was **two high**. Corrected here
from the board itself, and in phase 82.

### The four open rows

| row | disposition | why, measured today |
|---|---|---|
| **FB-005** — templates, curated first | ✅ **RIDES 0.2.2 (code) · closes on REL-001** | Both halves are shipped. The **editor** half (T1 install path, T5 share dialog) is committed and rides the tag by construction. The **platform** half is live: `/api/v1/community/templates` → **200**, and an invented sibling → **404**, so the 200 means *the route exists*. What is left is not code: the shelf reads **`total: 0`**. 🔴 **REL-001 closes this row** — publishing the association template puts the first row on an empty shelf, and needs **no release** |
| **FB-013** — chat | 🟡 **PARTLY RIDES · remainder ROLLS FORWARD, owner: phase 75** | **C4's launcher read half rides**: `3d01a44d` is an ancestor of HEAD, so the Chat tab is in the tree the tag is cut from. **C1/C2/C3 are deployed** — production stamps `91d8b0c4…`, `main`, `dirty: false`, 2026-08-28T06:48:29Z; `/api/v1/community/chat` and `/chat` are both **200**. ⬜ **C5 is NOT deployed and is one commit ahead of production** (`nodegx-community` HEAD `2bce720`), and ⬜ **`NODEGX_MODERATORS` is unset** — absent from every file in `/etc/nodegx-community/` and from the unit's `Environment=`. Neither is an app-cut concern. The launcher composer is unbuilt; free tags are unruled |
| **FB-009** — a syllabus entry you can start | 🧭 **BLOCKED ON RICHARD (prose) · ROLLS FORWARD, owner: phase 79** | [Phase 79 — the syllabus](../phase-79-the-syllabus/README.md) exists for exactly this and is **open** (T1 and T2 both open, lesson 1 built and driven). It outlives 0.2.2. The build half is not what is waiting |
| **FB-012** — default tutorials + share/export | 🧭 **BLOCKED ON RICHARD (content) · ROLLS FORWARD, owner: phase 75** | The format exists ([TUTORIAL-WORKSHOP.md](TUTORIAL-WORKSHOP.md)) and waits on the filled-in brief for tutorial #1. ⚠️ **Owner is this phase, deliberately** — phase 73 owns the *first* tutorial and closes when it ships, so naming it here would be `NONE` in disguise. P75 rolls forward past 0.2.2 by Richard's decision, so the row stays put and rolls **on purpose**, not by neglect |

### The phase-74 carry

| row | disposition | why |
|---|---|---|
| **FIX-025** | ✅ **RIDES (what landed) · §5's park ROLLS FORWARD** | §12 driven, §7 closed and driven; the fixes are committed editor code. §5 is a deliberate park (seeing it means signing out of Richard's live community session) and stays parked |
| **FIX-026** | 🧭 **BLOCKED ON RICHARD · ROLLS FORWARD, owner: phase 75** | The restore-source decision (a)/(b) is unanswered; nothing is buildable until it is |
| **FIX-027** | ✅ **RIDES (17/19/20) · remainder ROLLS FORWARD** | `f6d25d19` and `fada53fd` are both ancestors of HEAD. ⬜ Genuinely open: 14/15/16 🧭 · 22 🧭 · `state-on-a-page`, which **cannot be cleaned from this repo** and is a decision about where it lives |
| **`tsfixme` baseline** | 🧭 **BLOCKED ON RICHARD · ROLLS FORWARD, owner: phase 75** | A baseline decision, not code waiting to be written |
| **Prod `ANTHROPIC_API_KEY`** | 🧭 **BLOCKED ON RICHARD · ROLLS FORWARD · 🔴 NO DEADLINE** | Open, no clock — see the correction in this phase's handoff. ✅ **Its one real consequence is now GONE**: the cost-reporting staleness was `models.ts`, fixed 2026-08-31 as phase 82's REL-007 (`$2/$10` → `$3/$15`, `PRICING_AS_OF` moved). Nothing about the key breaks on 1 September |

### 🔴 Two board claims that measurement moved today

1. **"the chat routes are NOT DEPLOYED — `/api/v1/community/chat` is 404 on production"** (FB-013,
   session 57) **is no longer true.** A deploy landed 2026-08-28T06:48Z and the route answers **200**.
   The sentence remains a correct account of *why session 57 drove locally*; it is not a reading of
   production today. ✅ **A deployment claim is a stamp with a date on it, and it decays** — re-read
   it, do not inherit it.
2. **C5's undeployment is still true, and now it is measured rather than predicted.** Production is
   `91d8b0c`; `2bce720` sits one commit ahead in `nodegx-community`. ⚠️ **Anyone deploying anything
...
- ⬜ **FB-012** — a batch of default tutorials + share/export (content 🧭 Richard) (L) —
  🧭 **0.2.2: BLOCKED ON RICHARD (content) · ROLLS FORWARD, owner phase 75** (see the triage above) —
  ✅ **A WORKING FORMAT NOW EXISTS: [TUTORIAL-WORKSHOP.md](TUTORIAL-WORKSHOP.md)**, written 08-28 at
  Richard's request. He supplies the idea, the audience and the voice; the build, the steps, the
  derived starter and the gate are mine. **First batch of five proposed, led by his
  responsive-UI/"basic CSS" idea.** ⬜ Waiting on the filled-in brief for tutorial #1.
- 📄 **[YOUR-DECISIONS.md](YOUR-DECISIONS.md)** + **[WHAT-IS-A-BORDER-SWEEP.md](WHAT-IS-A-BORDER-SWEEP.md)**
  — 08-28, non-coder-facing. Every open question that is Richard's, in plain English, with a
  recommendation and a cost for each. ✅ **CORRECTED 2026-08-31: the `ANTHROPIC_API_KEY` item has NO
  deadline** — see the triage at the top of this file, and the handoff's own correction. The 🔥 that
  stood here fused two unrelated notes; its one real consequence (stale cost reporting) was fixed the
  same day as phase 82's REL-007.
- ⬜ **FB-009** — a syllabus entry you can actually start (D17 v0 hosting; lessons 🧭 Richard) (M/L)
  — 🧭 **0.2.2: BLOCKED ON RICHARD (prose) · ROLLS FORWARD, owner [phase 79](../phase-79-the-syllabus/README.md)**, which is open and exists for this.

## Tier 4 — new scope (rulings landed 08-22; scoping docs first)

- 🟡 **FB-005** — templates, **curated first** (L+) — ✅ **0.2.2: RIDES (code) · CLOSES ON REL-001.** Measured 2026-08-31: `/api/v1/community/templates` **200** with `total: 0` beside a **404** negative control — the shelf is real and empty, so the gap is content and phase 82's REL-001 is what fills it. — **T1–T5 BUILT AND NOW DEPLOYED; the blocker is CONTENT** — **R-templates ruled 08-22**: share files a
...
- 🟡 FIX-025 — ✅ **0.2.2: RIDES (what landed) · §5's park ROLLS FORWARD.** **This line was stale in all three places, corrected 2026-08-27 (s56).** §12 was
...
- 🧭 FIX-026 — restore source decision (a)/(b), then build — 🧭 **0.2.2: BLOCKED ON RICHARD · ROLLS FORWARD, owner phase 75**
- 🟡 FIX-027 — ✅ **0.2.2: RIDES (§17/§19/§20 — `f6d25d19` and `fada53fd` are ancestors of HEAD) · REMAINDER ROLLS FORWARD.** **This line was stale on THREE of its five entries, corrected 2026-08-28 (s59).**
...
- 🧭 `tsfixme` baseline decision — 🧭 **0.2.2: BLOCKED ON RICHARD · ROLLS FORWARD, owner phase 75**
- 🧭 Prod `ANTHROPIC_API_KEY` — 🧭 **0.2.2: BLOCKED ON RICHARD · ROLLS FORWARD · 🔴 NO DEADLINE.**
  ~~⚠️ intro pricing ends 2026-08-31~~ — a **price date, not a deadline**. Nothing breaks and getting
  a key first changes nothing; the one consequence was stale cost reporting, and that was fixed
  2026-08-31 as phase 82's REL-007 (`models.ts` `$2/$10` → `$3/$15`).
