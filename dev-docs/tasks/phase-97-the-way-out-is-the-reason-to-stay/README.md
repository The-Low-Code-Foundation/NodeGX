...
## 4. Rulings needed — ✅ ALL SEVEN RULED (R7 taken 2026-09-20, s10)
...
| **BRG-D7** 🔴 | **`IStorageSchema` is synchronous, and a socket cannot serve it.** BRG-002 de-synchronised the facade and left the schema surface as it was, correctly — every caller was synchronous. On PostgreSQL the adapter serves it from a per-process model with queued DDL and a barrier before every data call (BRG-005 §7.1), which is truthful under R2's one-process bound and declared as three divergences. The fix is BRG-002's method applied to six callers (`byob-admin`, `RoleStore`, `identities`, `security/state.rolesForUser`, `SearchIndexer`, `backup`) | BRG-006, or a task of its own |

| **BRG-D8** 🔴 | **A `Boolean` column reads back as `0`/`1` on SQLite and `false`/`true` on PostgreSQL — the same app, the same record, two different JSON values over HTTP.** Found by BRG-004 s9's verification, which is the first thing that ever read the same rows back through both facades. Neither adapter is wrong where it looks: both apply the declared type in `_rowToRecord` by reading `schema.properties[key].type`, and `SchemaManager.getTableSchema()` returns a `TableSchema`, which has **no `properties` member** — so on a service-opened backend (`BackendService` passes no `collections`) the declared type is never seen on either side and each driver wins. 🔴 **BRG-003's suite has no case that round-trips a boolean**, which is why 56/56 said nothing about it ([[a-gate-can-have-a-hole-shaped-like-the-defect]]). Declared as `types/boolean-reads-as-0-1-on-sqlite`, measured at the facade AND over HTTP (`brg-004-data-plane.test.ts`), and deliberately NOT repaired here: making them agree changes what every existing SQLite app reads back, which is a product decision | BRG-006, or a task of its own — **Richard's call on which way they should agree** |
...
| **BRG-D10** 🔴 | **Two REST surfaces answer differently for the same Boolean, in the same row, on the same engine.** `GET /api/:c` reads `1` where `GET /classes/:c` reads `true` — **on SQLite, with no PostgreSQL anywhere near it**, from rows written either way. On PostgreSQL every reader agrees, and a NodeGX graph's own `Query Records` node already reads `true` on both engines. Found by BRG-006's drive, which is the first thing that ever read one column three ways on two engines. 🔴 **BRG-D8 is one cell of this table**, and this reframes the ruling it waits on: the case for keeping SQLite's `0`/`1` is "every existing app reads that", which is false as stated — only `/api` on SQLite does. ⚠️ Not only app collections: `migrate`'s verify report names `_User.emailVerified`, `_Files.private` and `_ApiKey.revoked` too. **Not repaired** — it is a product defect the bridge did not cause, and which way the two surfaces should agree is the same decision BRG-D8 is waiting on | BRG-006 filed it — **Richard's call, together with BRG-D8** |
