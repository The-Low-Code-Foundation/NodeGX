...
| [BRG-005](BRG-005-THE-POSTGRES-ADAPTER.md) | `PostgresAdapter` implementing the BRG-001 interface until BRG-003 is green | 🏗 s6-s7 — **the driver (R6), the pool, the geo port, and the dialect seam inside `QueryBuilder`**; 287 adapter tests, 34 of them on both engines over one corpus | ✅ **AC5 AC6 AC8**; 🟡 AC2 AC4 AC7 | ⬜ AC1 AC3 AC9 — all three need the adapter class, and **nothing is now in front of it** |
