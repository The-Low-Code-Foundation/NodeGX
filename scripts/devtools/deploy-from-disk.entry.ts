...
import { readWireHealth } from '../../packages/noodl-preview/src/wireHealth';
...
/**
 * D44 · P88 GAM-024 — the port pass and the health reading live in product code now
 * (`noodl-preview/src/wireHealth.ts`), because `nodegx deploy` runs the same pass. Two copies were
 * the drift GAM-024 §4 warned about. This devtool differs only in keeping the filter ON, so its
 * bundle DROPS what the deploy names.
 */
...
async function main() {
  ...
  // 🔴 P88 GAM-024 s18: without this, `readWireHealth` making the project the editor instance armed the
  // autosave, and a census wrote a legacy `project.json` into every template folder it deployed.
  ...
  // 🔴 The editor registers the open project as a node-library module; nothing headless does, and
  // `evaluateHealth()` bails on exactly that guard. `readWireHealth` runs the port pass FIRST (D44),
  // then registers the project and leaves it registered, so the export below filters for real.
  const wireHealth = await readWireHealth(project, {
    leaveFilterOn: true,
    arms: {
      skipAdapters: has('--arm-skip-adapters'),
      skipImport: has('--arm-skip-import'),
      skipImportComplete: has('--arm-skip-import-complete')
    }
  });
  const dynPorts = wireHealth.ports;
  ...
  for (const [name, before] of Object.entries(onGraph)) {
    ...
    if (after !== before)
      dropped.push({ component: name, onGraph: before, inDeployed: after, dropped: before - after });
  }
  ...
  const report = {
    ...
    // P88 GAM-024 AC2 — the wires, not only the counts. Broken and unchecked are both dropped here.
    droppedWires: [
      ...wireHealth.broken,
      ...wireHealth.unchecked.map((w) => ({ ...w, reason: 'unchecked: kit type not loaded' }))
    ],
    brokenWires: wireHealth.broken,
    uncheckedWires: wireHealth.unchecked,
    uncheckedTypes: wireHealth.uncheckedTypes,
  ...
  if (has('--json')) process.stdout.write(JSON.stringify(report, null, 2) + '\n');
    ...
    process.stdout.write(
      `dynamic ports        ${report.dynamicPorts.portsAdded} onto ${report.dynamicPorts.nodesPorted} nodes` +
        ` (${report.dynamicPorts.types.join(', ') || 'no lazy types'})` +
        (report.dynamicPorts.unmatched ? ` \u26a0 ${report.dynamicPorts.unmatched} unmatched` : '') +
        '\n'
    );
    process.stdout.write(
      `components evaluated ${report.componentsEvaluated} guards=${JSON.stringify(report.healthGuards)}\n`
    );
    ...
    process.stdout.write(
      `sabotage             ${JSON.stringify(report.sabotageCounts)} on ${report.sabotageComponent}\n`
    );
    process.stdout.write(`dropped by component ${JSON.stringify(report.droppedByComponent)}\n`);
    for (const w of report.droppedWires)
      process.stdout.write(`  dropped  ${w.component}: ${w.from} → ${w.to} (${w.reason})\n`);
